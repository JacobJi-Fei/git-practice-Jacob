...
var sevenShape = [[0, 0], [0, -1], [1, 0], [2, 0]];
...
function generatePiece() {
    ...
    // initializePiece 这个函数是需要被完善的 (内部的 blockList 和其他 attributes 的实现)
...
function moveLeft(piece) {
    ...
    *************************/
    ...
    if(feasibility){
        piece.colPos -= 1;
        piece.style.left = piece.colPos * BLOCKSIZE + 'px';
        for(var i = 0;i < 4;i++){
            blockList[i].colPos -= 1;
            blockList[i].style.left = blockList[i].colPos * BLOCKSIZE + 'px';
        }
    }  
    /************************
        YOUR CODE ENDS
...
function moveRight(piece) {
    ...
    *************************/
    ...
    for(var i = 0; i < 4; i++)
    {
        if(blockList[i].colPos == 9 || occupationMatrix[blockList[i].rowPos][blockList[i].colPos+1]!= null)
        {
            feasibility = false;
            break;
        }
...
function pieceMoveDown() {
    ...
    *************************/
    ...
    var TorF = isMoveDownValid(piece);
    ...
    else
    {
        ...
        var blockList = piece.children;
        for(var i = 0;i < 4;i++){
            blockList[i].rowPos += 1;
            blockList[i].style.top = blockList[i].rowPos * BLOCKSIZE + 'px';
        }
    }
...
function isMoveDownValid(piece) {
    ...
    */
    ...
    /************************
        YOUR CODE STARTS
    *************************/
    console.log(piece.rowPos);
    var i;
    for(i=0; i<4; i++)
    {
        if(piece.children[i].rowPos == 19 || occupationMatrix[piece.children[i].rowPos+1][piece.children[i].colPos] != null)
        {
            return false;
        }
    }
...
function rotateClock(piece) {
    ...
    *************************/
        var blockList = piece.children;
        var xCen = blockList[0].rowPos;
        var yCen = blockList[0].colPos;
        var feasible = true;
        var attemptPos = new Array();
        for(var i = 0; i<4; i++){
            attemptPos[i] = new Array();
            if(i == 0){
                attemptPos[i][0] = xCen;
                attemptPos[i][1] = yCen;
            }
            else{
                var xPre = blockList[i].rowPos;
                var yPre = blockList[i].colPos;
                var xCur = xCen - yCen + yPre;
                var yCur = xCen + yCen - xPre;
                if(yCur < 0 || yCur > 9 || xCur>19 || occupationMatrix[xCur][yCur]!=null){
                    feasible = false;
                    break;
                }
                attemptPos[i][0] = xCur;
        ...
        if(feasible){
            for(i = 0;i < 4;i++){
                blockList[i].rowPos = attemptPos[i][0];
                blockList[i].colPos = attemptPos[i][1];
                blockList[i].style.top = blockList[i].rowPos * BLOCKSIZE + "px";
                blockList[i].style.left = blockList[i].colPos * BLOCKSIZE + "px";
            }
            if(isMoveDownValid(piece) == false){
                updateOccupationMatrix(piece);
                piece.id = "";
                check();
                generatePiece();
...
function clearRow(rowNumber) {
    ...
    *************************/
    for(var i = 0; i < COL_NUM; i++){
        var Node = occupationMatrix[rowNumber][i];
...
document.onkeydown = function (event) {
    ...
    *************************/
    var piece = document.getElementById("currentPiece");
    var instruct = event.keyCode;
    switch(instruct){
        case 37:    moveLeft(piece);        break;
        case 39:    moveRight(piece);       break;
        case 40:    fallToBottom(piece);    break;
        case 38:    rotateAntiClock(piece); break;
        case 68:    rotateClock(piece);     break;
    }
    /************************
        YOUR CODE ENDS
    *************************/
...
function play() {
    ...
    *************************/
    cleanBoard();
    generatePiece();
    var newPiece = document.getElementById('currentPiece');
